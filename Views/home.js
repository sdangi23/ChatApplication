...
async function sendMsgGroup(grpId){
        ...
        try{
        const result = await axios.post(`http://54.145.181.21:3000/savemsg?id=${grpId}`, msgdetails, { headers: { "Authorization": token } })
...
async function saveMsg(msgdetails, token){
      axios.post('http://54.145.181.21:3000/savemsg', msgdetails, { headers: { "Authorization": token } })
      .then((result) => {
        alert(result.data.message); 
        if(result.status === 201){   
            document.getElementById("text-content").value="";
...
async function showAllMsg(){
    const token = localStorage.getItem('token');
    const dbmsgs = await axios.get('http://54.145.181.21:3000/getmsg' , { headers: { "Authorization": token } });
...
async function updateMsg(){
    ...
    const token = localStorage.getItem('token');
    const dbmsgs = await axios.get(`http://54.145.181.21:3000/updatemsg?id=${lastId}` , { headers: { "Authorization": token } });
...
async function showMemberGroups() {
  ...
  const token = localStorage.getItem('token');
  const dbout = await axios.get('http://54.145.181.21:3000/getGrps' , { headers: { "Authorization": token } });
...
async function showAvailableUsers(){
  ...
  const dbout = await axios.get('http://54.145.181.21:3000/getusers');
...
async function addToGrp(uId){
  ...
  const token = localStorage.getItem('token');
  const result = await axios.post('http://54.145.181.21:3000/createGrp', groupDetails, { headers: { "Authorization": token } })
  alert(result.data.message);
}
...
async function showGroupMsgs(grpId) {
    ...
    const token = localStorage.getItem('token');
    const dbmsgs = await axios.get(`http://54.145.181.21:3000/getmsg?id=${grpId} `, { headers: { "Authorization": token } });
    ...
    const resultdb = await axios.get(`http://54.145.181.21:3000/getusers/?id=${grpId}`);
...
async function removeUser(uId , grpId){
  ...
  const token = localStorage.getItem('token');
  const result = await axios.post(`http://54.145.181.21:3000/removeuser` , {uId: uId , grpId: grpId} , { headers: { "Authorization": token } } )
  alert(result.data.message);
}